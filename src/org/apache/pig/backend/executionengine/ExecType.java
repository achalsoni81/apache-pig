...
public interface ExecType extends Serializable {
 ...
 /**
  * Returns the corresponding value in the legacy ExecType enum. This method is
  * for maintaining backwards compatibility for ExecType that relies on ExecType
  * being an enum. The legacy enum ExecType should NOT be used. Instead, all
  * properties of the ExecType the script is associated with can be exposed by 
  * adding methods to sub-interfaces of the new ExecType and then calling these methods 
  * in the code to determine what actions to take.
  */
 public org.apache.pig.ExecType getLegacyExecType();

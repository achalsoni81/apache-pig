...
public class MapReduceLauncher extends Launcher{
    ...
    public PigStats launchPig(PhysicalPlan php,
        ...
        while(mrp.size() != 0) {
            ...
            if(jc == null) {
                ...
                for(MapReduceOper mro: roots) {
                    if(mro instanceof NativeMapReduceOper) {
                        ...
                        try {
                            ((MRScriptState) ScriptState.get()).emitJobsSubmittedNotification(1);
                            natOp.runJob();
                            numMRJobsCompl++;
                        } catch (IOException e) {
            ...
            ((MRScriptState) ScriptState.get()).emitJobsSubmittedNotification(jobsWithoutIds.size());
                  ...
                  try { jcThread.join(sleepTime); }
                	...
                	for(Job job : jobsWithoutIds){
                		if (job.getAssignedJobID() != null){
                            ...
                            if (mro != null) {
                                String alias = ((MRScriptState) ScriptState.get()).getAlias(mro);
                                log.info("Processing aliases " + alias);
                                String aliasLocation = ((MRScriptState) ScriptState.get()).getAliasLocation(mro);
                			...
                			((MRScriptState) ScriptState.get()).emitJobStartedNotification(
        ...
        ((MRScriptState) ScriptState.get()).emitProgressUpdatedNotification(100);
        ...
        // MRPigStatsUtil.stopCollection also computes the return code based on
        ...
        if (succJobs != null) {
            for (Job job : succJobs) {
                ...
                for (POStore st : sts) {
                    if (pc.getExecType() instanceof LocalExecType) {
        ...
        PigStats pigStats = MRPigStatsUtil.getPigStats(ret);
    ...
    private boolean notifyProgress(double prog, double lastProg) {
        if (prog >= (lastProg + 0.04)) {
            ...
            if(perCom!=100) {
                ...
                ((MRScriptState) ScriptState.get()).emitProgressUpdatedNotification(perCom);
            }
            return true;
        }
        return false;
